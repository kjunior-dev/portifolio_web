...
import {IExperienceTemplate} from "@/components/template/ExperienciaTemplate/ExperienceTemplate";
        ...
        {expProf?.experiencias.map((item) => (
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
              ...
              <div className="flex flex-wrap content-start gap-2">
                {item.tecnologias.map((technology) => (
                  <TechBadge key={technology?.ordem}>{technology?.nome}</TechBadge>
                ))}
              </div>
            </div>
          </article>
